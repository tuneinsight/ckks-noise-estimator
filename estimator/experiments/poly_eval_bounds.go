...
func main() {
	...
	wantInt, wantCmplx := Eval(seed, truncVal, k, est, ecd, enc, dec, polyEval, poly)

	for i := 0; i < 1024; i++ {

		fmt.Println(i)
		
		haveInt, haveCmplx := Eval(seed, truncVal, k, est, ecd, enc, dec, polyEval, poly)

		stats := hefloat.GetPrecisionStats(params, ecd, dec, haveCmplx, wantCmplx, 0, false)
...
func Eval(seed int64, trunc *big.Int, k float64, est estimator.Estimator, ecd *hefloat.Encoder, enc *rlwe.Encryptor, dec *rlwe.Decryptor, polyEval *hefloat.PolynomialEvaluator, poly hefloat.Polynomial) ([]*big.Int, []*big.Float){

	want, _, _, ct := est.NewTestVectorFromSeed(ecd, enc, complex(-k+1, 0), complex(k-1, 0), seed, trunc)
