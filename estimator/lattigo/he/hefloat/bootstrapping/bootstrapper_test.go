...
func TestBootstrapping(t *testing.T) {
	...
	t.Run("BootstrappingWithoutRingDegreeSwitch", func(t *testing.T) {
		...
		t.Run("Bootstrapping", func(t *testing.T) {
			...
			hefloat.VerifyTestVectors(params, ecd, dec, values, ctQL, params.LogDefaultScale()-10, 0, *printPrecisionStats, t)
		})
	})
	...
	t.Run("BootstrappingPackedWithRingDegreeSwitch", func(t *testing.T) {
		...
		for i := range cts {
			...
			hefloat.VerifyTestVectors(params, ecd, dec, utils.RotateSlice(values, i), &cts[i], params.LogDefaultScale()-10, 0, *printPrecisionStats, t)
		}
	})
	...
	t.Run("BootstrappingWithRingTypeSwitch", func(t *testing.T) {
		...
		t.Run("ConjugateInvariant->Standard->Bootstrapping->Standard->ConjugateInvariant", func(t *testing.T) {
			...
			hefloat.VerifyTestVectors(params, ecd, dec, values, ctLeftQL, params.LogDefaultScale()-10, 0, *printPrecisionStats, t)

			require.True(t, ctRightQL.Level() == params.MaxLevel())
			require.True(t, ctRightQL.Scale.Equal(params.DefaultScale()))

			hefloat.VerifyTestVectors(params, ecd, dec, values, ctRightQL, params.LogDefaultScale()-10, 0, *printPrecisionStats, t)
		})
	})
}
